...
(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.ergogen = factory(global.makerjs, global.jsyaml, global.jszip, global.math, global.kle, global.axios));
})(this, (function (require$$0, require$$1$1, require$$1$2, require$$3, require$$1, require$$0$1) { 'use strict';
	...
	var point = class Point {
	    ...
	    rect(size=14) {
	        let rect = u$9.rect(size, size, [-size/2, -size/2]);
	...
	kle$2.convert = (config, logger) => {
	    ...
	    for (const key of keyboard.keys) {
	        ...
	        converted.columns[colid].rows[rowid] = u$8.deepcopy(meta);
	...
	points.visualize = (points, units) => {
	    ...
	    for (const [pname, p] of Object.entries(points)) {
	        ...
	        const rect = u$5.rect(w, h, [-w/2, -h/2]);
	...
	const similar = (keys, reference, name, units) => {
	    ...
	    const external_tester = (point, key) => {
	        const value = u$4.deep(point, key);
	...
	const binding = (base, bbox, point, units) => {
	    ...
	    if (bind[0] || bind[1]) base = u$3.union(base, u$3.rect(br, bt));
	    if (bind[1] || bind[2]) base = u$3.union(base, u$3.rect(br, -bd, [0, bd]));
	    if (bind[2] || bind[3]) base = u$3.union(base, u$3.rect(-bl, -bd, [bl, bd]));
	    if (bind[3] || bind[0]) base = u$3.union(base, u$3.rect(-bl, bt, [bl, 0]));
	...
	const circle = (config, name, points, outlines, units) => {
	    ...
	    // return shape function and its units
	    return [() => {
	        let circle = u$3.circle([0, 0], radius);
	...
	const polygon = (config, name, points, outlines, units) => {
	    ...
	    return [point => {
	        ...
	        let poly = u$3.poly(parsed_points);
	        const bbox = u$3.bbox(parsed_points);
	        return [poly, bbox]
	    }, units]
	};
	...
	const outline = (config, name, points, outlines, units) => {
	    ...
	    // return shape function and its units
	    return [() => {
	        let o = u$3.deepcopy(outlines[config.name]);
	...
	outlines.parse = (config, points, units) => {
	    ...
	    for (let [outline_name, parts] of Object.entries(config)) {
	        ...
	        for (let [part_name, part] of Object.entries(parts)) {
	            ...
	            const operation = u$3[a$3.in(part.operation || 'add', `${name}.operation`, ['add', 'subtract', 'intersect', 'stack'])];
	            ...
	            if (expand) {
	                outlines[outline_name] = m$5.model.outline(
	                    outlines[outline_name], Math.abs(expand), joints, (expand < 0), {farPoint: u$3.farPoint}
	                );
	            }
...
`
	...
	const footprint = pcbs._footprint = (points, net_indexer, component_indexer, units, extra) => (config, name, point) => {
	    ...
	    let params = u$2.deepcopy(original_params);
	    delete params.mirror;
	    // but still override with it when applicable
	    if (point.meta.mirrored && original_params.mirror !== undefined) {
	        const mirror_overrides = a$1.sane(original_params.mirror, `${name}.params.mirror`, 'object')();
	        params = prep.extend(params, mirror_overrides);
	    }
	    a$1.unexpected(params, `${name}.params`, Object.keys(fp.params));
	    ...
	    for (const [param_name, param_def] of Object.entries(fp.params)) {
	        ...
	        let def_type = a$1.type(param_def)(units);
	        if (def_type == 'string') {
	            parsed_def = {type: 'string', value: param_def};
	        } else if (def_type == 'number') {
	            parsed_def = {type: 'number', value: a$1.mathnum(param_def)(units)};
	        ...
	        if (a$1.type(value)() == 'string') {
	            value = u$2.template(value, point.meta);
	        ...
	        if (['string', 'number', 'boolean', 'array', 'object'].includes(type)) {
	            ...
	            const net = a$1.sane(value, `${name}.params.${param_name}`, 'string')(units);
	            const index = net_indexer(net);
	            parsed_params[param_name] = net_obj(net, index);
	        } else { // anchor
	            let parsed_anchor = anchor$1(value, `${name}.params.${param_name}`, points, point)(units);
	...
	pcbs.parse = (config, points, outlines, units) => {
	    ...
	    for (const [pcb_name, pcb_config] of Object.entries(pcbs)) {
	        ...
	        const references = a$1.sane(pcb_config.references || false, `pcbs.${pcb_name}.references`, 'boolean')();
	        const template = template_types$1[a$1.in(pcb_config.template || 'kicad5', `pcbs.${pcb_name}.template`, Object.keys(template_types$1))];

	        // outline conversion
	        if (a$1.type(pcb_config.outlines)() == 'array') {
	            pcb_config.outlines = {...pcb_config.outlines};
	        }
	        const config_outlines = a$1.sane(pcb_config.outlines || {}, `pcbs.${pcb_name}.outlines`, 'object')();
	        const kicad_outlines = {};
	        for (const [outline_name, outline] of Object.entries(config_outlines)) {
	            const ref = a$1.in(outline.outline, `pcbs.${pcb_name}.outlines.${outline_name}.outline`, Object.keys(outlines));
	            const layer = a$1.sane(outline.layer || 'Edge.Cuts', `pcbs.${pcb_name}.outlines.${outline_name}.outline`, 'string')();
	...
	var mod_parser = { parseContent: parseContent$1 };
