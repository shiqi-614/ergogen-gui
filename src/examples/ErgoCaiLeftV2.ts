import {ConfigExample} from "./index";

const ErgoCaiLeftV2 : ConfigExample = {
    label: "ErgoCai_LeftV2",
    author: "jinsongc",
    value: `
units:
  kx: cx
  ky: cy
  px: kx + 3
  py: ky + 3

points:
  zones:
    matrix:
      columns:
        col6:
          key:
            width: 1.5kx
            shift: [-0.02kx, 0]
          rows:
            row5:
              width: 1kx
              shift: [0.24kx, 0]
        col5:
          key:
            spread: 1.3kx 
        col4:
          key:
            stagger: 2.5
        col3:
          key:
            stagger: 2.5
        col2:
          key:
            stagger: -2.5
        col1:
          key:
            stagger: -2.5
          rows.row5.skip: true
        col0: 
          rows.row5.skip: true
          rows.row3.skip: true
          rows.row2: 
            width: 1.5kx
            rotate: 90
            shift: [0, -0.27kx]
          rows.row4:
            width: 1.5kx
            rotate: 90
            shift: [0, 0.25kx]
      rows:
        row5:
        row4:
        row3:
        row2:
        row1:
    thumbs:
      anchor:
        ref: matrix_col1_row5
        shift: [1.37kx, -1.70kx]
        rotate: -25
      columns:
        left:
          rows:
            row1: 
            row2: 
              width: 2kx
              rotate: -90
              shift: [-0kx, -0.54kx]
            row3:
              width: 2kx
              rotate: -90
              shift: [-1.15kx, 0.52kx]
        right:
      rows:
        row3:
        row2:
        row1:        

outlines:
  raw:
    - what: rectangle
      where: true
      size: [kx, ky]
  keys:
    - what: rectangle
      where: true
      size: [kx-0.5,ky-0.5]
  board:
    - what: polygon
      operation: stack
      points:
        - ref: matrix_col6_row1
          shift: [-0.8px, 1.6py]
        - ref: matrix_col3_row1
          shift: [-0.3px, 1.6py - 5]
        - ref: matrix_col3_row1
          shift: [-0.3px, 1.6py - 2]
        - ref: matrix_col3_row1
          shift: [0.3px, 1.6py - 2]
        - ref: matrix_col3_row1
          shift: [0.3px, 1.6py - 5]
        - ref: matrix_col1_row1
          shift: [0.61px, 1.6py ]
        - ref: matrix_col1_row1
          shift: [0.61px, 1.6py + 3]
        - ref: matrix_col1_row1
          shift: [1.2px, 1.6py + 3]
        - ref: matrix_col1_row1
          shift: [1.2px, 1.6py]
        - ref: matrix_col1_row1
          shift: [1.6px, 1.6py]
        - ref: matrix_col1_row4
          shift: [1.6px, 0.1py]
        - ref: thumbs_right_row1
          shift: [0.6px, 0.6py + 1]
        - ref: thumbs_right_row3
          shift: [0.6px,-0.6py]
        - ref: matrix_col4_row5
          shift: [0.8px, -0.6py - 4.5]
        - ref: matrix_col5_row5
          shift: [0.5px,-0.7py]
        - ref: matrix_col6_row5
          shift: [-0.8px - 0.25kx,-0.7py]
      fillet: 1
  combo:
    - name: board
    - operation: subtract
      name: keys

pcbs:
  ergo_cai_left: 
    outlines:
      main:
        outline: board
    footprints:
      keys1:
        what: 
          github:
            repo: "shiqi-614/ErgoCaiLib"
            file: "footprints/ErgoCai.pretty/SW_MX_HS_CPG151101S11_1u.kicad_mod"
        where: true
      keys2:
        what: 
          github:
            repo: "shiqi-614/ErgoCaiLib"
            file: "footprints/ErgoCai.pretty/SW_MX_HS_CPG151101S11_1.5u.kicad_mod"
        where: [matrix_col0_row2, matrix_col0_row4, matrix_col6_row1, matrix_col6_row2, matrix_col6_row3, matrix_col6_row4]
      keys3:
        what: 
          github:
            repo: "shiqi-614/ErgoCaiLib"
            file: "footprints/ErgoCai.pretty/SW_MX_HS_CPG151101S11_2u.kicad_mod"
        where: [thumbs_left_row2, thumbs_left_row3]
      diodes:
        what: 
          github:
            repo: "shiqi-614/ErgoCaiLib"
            file: "footprints/ErgoCai.pretty/Diode-Dual.kicad_mod"
        where: true
        adjust:
          shift: [0, -8.4]
          rotate: 180
        side: back
      stab:
        what:
          github:
            repo: "shiqi-614/ErgoCaiLib"
            file: "footprints/ErgoCai.pretty/STAB_MX_2u.kicad_mod"
        where: [thumbs_left_row2, thumbs_left_row3]
    modules:
      battery:
        what:
          github:
            repo: "shiqi-614/ErgoCai.modules"
            file: "battery/battery_all_back/battery_all_back.kicad_pcb"
        where:
          ref: [matrix_col5_row1]
          shift: [-4.5, 22]
          rotate: -90
        footprints:
          BAT_SW1:
            where:
              ref: [matrix_col3_row1]
              shift: [0, 27]
              rotate: 180
      usb:
        what: 
          github:
            repo: "shiqi-614/ErgoCai.modules"
            file: "usb/default_usb/default_usb.kicad_pcb"
        where:
            ref: matrix_col0_row1
            shift: [0, 32]
      mcu:
        what: 
          github:
            repo: "shiqi-614/ErgoCai.modules"
            file: "mcu/nRF52840_holyiot_18010/nRF52840_holyiot_18010.kicad_pcb"
        where:
          ref: matrix_col2_row1
          shift: [12, 20]
          rotate: 0
        side: back  
      reset:
        what: 
          github: 
            repo: "shiqi-614/ErgoCai.modules"
            file: "reset/reset/reset.kicad_pcb"
        where: 
          ref: [matrix_col0_row1]
          shift: [0, 18]
          rotate: 90 
      led:
        what: 
          github:
            repo: "shiqi-614/ErgoCai.modules"
            file: "led/RGB_WS2812B_3/RGB_WS2812B_3.kicad_pcb"
        where: 
          ref: [matrix_col0_row1]
          shift: [0, 18] 
          rotate: 0
    `
};

export default ErgoCaiLeftV2;
